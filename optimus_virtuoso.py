...
Original file is located at
    https://colab.research.google.com/drive/1lYDQvzAIw-MRb9BUx7jL86ou9SvSulsg
...
#@title Convert TXT dataset to Char Token TXT dataset

#@markdown NOTE: Tokenizer can't handle large datasets due to limited number of chars in UTF-8 charmap
full_path_to_processed_TXT_dataset = "/content/Optimus-VIRTUOSO-Music-Dataset.txt" #@param {type:"string"}

TXTT, DIC, RDIC = TMIDI.Tegridy_TXT_Tokenizer(open(full_path_to_processed_TXT_dataset).read())

# Writing dataset to TXT file
with open('/content/Optimus-VIRTUOSO-Char-Token-TXT-Music-Dataset' + '.txt', 'wb') as f:
  f.write(TXTT.encode('utf-8', 'replace'))
  f.close
...
full_path_to_training_text_file = "/content/Optimus-VIRTUOSO-Char-Token-TXT-Music-Dataset.txt" #@param {type:"string"}
...
creativity_temperature = 0.35 #@param {type:"slider", min:0.05, max:4, step:0.05}
top_k_prob = 16 #@param {type:"slider", min:0, max:128, step:1}
input_prompt = "\u05D9\u05D4\u05D5\u05D4" #@param {type:"string"}
...
#@title Detokenize Char Token output
full_path_to_processed_TXT_dataset = "/content/Optimus-VIRTUOSO-Music-Dataset.txt" #@param {type:"string"}
TXTT, DIC, RDIC = TMIDI.Tegridy_TXT_Tokenizer(open(full_path_to_processed_TXT_dataset).read())
RTXT = TMIDI.Tegridy_TXT_DeTokenizer(completion, RDIC)
...
Char_Token_encoding = True #@param {type:"boolean"}
...
chars_encoding_offset_used_for_dataset = 196 #@param {type:"number"}
